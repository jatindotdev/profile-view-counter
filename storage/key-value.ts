...
export class KeyValue<T> {
  ...
  get(key: key | key[]): (T | undefined) | (T | undefined)[] {
    if (Array.isArray(key)) {
      const getData = (_key: key) => this.data[_key];
  ...
  set(key: any, value?: any) {
    if (typeof value == "function") {
      const newValue = value(this.data[key]);
      if (newValue !== undefined) this.data[key] = newValue;

      return newValue;
    }
    if (typeof key == "string") return (this.data[key] = value);
    if (Array.isArray(key)) Object.assign(this.data, Object.fromEntries(key));
    else if (typeof key == "object") Object.assign(this.data, key);
  }
  has(key: key) {
  ...
  forEach(callbackFn: (value: T, key: key, database: keyValue<T>) => void) {
